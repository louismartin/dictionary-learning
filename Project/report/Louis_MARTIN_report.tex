...
Given a signal of size $m$, $x \in \RR^m$, a dictionary is a collection of $k$ prototype signal atoms represented in a matrix $D \in \RR^{m \times k}$
We want to find $\alpha \in \RR^k$, the sparse representation of $x$ in the dictionary $D$.
We want an exact representation $D \alpha = x$ or more frequently an approximate representation $x \approx D \alpha$.
